...
### Hyper-parameter optimization ###
  #Import data
train_eps = as.matrix(read.csv(sprintf('%s//panel_SFA_application_Results//panel_SFA_finnish_electricty_application_NN_train_eps.csv', project_filepath), header = TRUE))
train_u = as.matrix(read.csv(sprintf('%s//panel_SFA_application_Results//panel_SFA_finnish_electricty_application_NN_train_u.csv', project_filepath), header = TRUE))
N = length(train_u)
T = ncol(train_u)
...
test_eps = as.matrix(read.csv(sprintf('%s//panel_SFA_application_Results//panel_SFA_finnish_electricty_application_NN_test_eps.csv', project_filepath), header = TRUE))
N = length(test_eps)
T = ncol(test_eps)

u_hat = matrix(0, N, T)
V_u_hat = matrix(0, N, T)
#Fit a LL forest model to predict u_it using all eps_i1,  ..., eps_iT
for (t in c(1:T)){
  ...
  V_u_hat[1:nrow(V_u_hat), t] = V_u_hat_t
}

#Save the MSE in the simulation results file 
write.csv(u_hat, sprintf('%s//panel_SFA_application_Results//finnish_electricity_LLF_Gaussian_copula_u_hat.csv', project_filepath), row.names = FALSE)
write.csv(V_u_hat, sprintf('%s//panel_SFA_application_Results//finnish_electricity_LLF_Gaussian_copula_V_u_hat.csv', project_filepath), row.names = FALSE)
